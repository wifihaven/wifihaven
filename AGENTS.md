...
The SPA's `QueryClient` is one process-wide singleton that outlives a session, so an unscoped key is a cache entry two households SHARE — that is #2603, where household B's session rendered household A's devices/profiles for the full five-minute `staleTime`. Never write an inline array key: every key comes from `qk` (`web/src/api/queryKeys.ts`), which `withHouseholdScope` prefixes with the JWT's `hh` claim. Covers `queryKey:`, `get`/`setQueryData`, and bare invalidation prefixes. `useAuth` also `queryClient.clear()`s on login and logout (the primary fix); the scoping is the defence in depth, so it only holds if complete. Enforced by `no-restricted-syntax` in `web/.eslintrc.cjs`. → see [`docs/process/query-cache-scoping.md`](docs/process/query-cache-scoping.md)
