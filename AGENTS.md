...
### Grafana Cloud stack
`https://wifihaven.grafana.net` hosts app metrics (Alloy), Render infra OTLP, deploy annotations. Repo secrets `GRAFANA_CLOUD_URL` + `GRAFANA_CLOUD_ANNOTATION_TOKEN`. The API also ships app logs to Grafana Cloud **Loki** — to **query** them for debugging/incidents (LogQL via Explore UI; label set `service`/`env`/`level` + structured metadata), prefer Loki over `render logs` (a fallback). → see [`docs/ops/grafana-cloud.md`](docs/ops/grafana-cloud.md) (§ *Querying logs from Loki* `{#querying-logs}`)
