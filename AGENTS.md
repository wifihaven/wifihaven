...
What WifiHaven is; repo layout; the **deployment model** (self-hosted = the customer's own hardware, free forever; cloud = ours at app./api.wifihaven.net — the direction inverts if you state it from memory, #2718) and the SPA hosting split behind it (self-hosted bundles SPA into API, cloud uses Cloudflare Pages); key API surface (`/api/router/*`, `/api/blocklists/*`); domain vocabulary (Profile, Device, Schedule, TimeLimit, App, App time limit, app exempt-from-daily, TimeUsage, TimeExtension, BlocklistDomain, QueryLog, Location); server-side policy decision pipeline (paused → schedule → daily limit → app limit → manual → extraBlocked → extraAllowed → blocklists → blockIpOnly). → see [`docs/architecture.md`](docs/architecture.md)

### "Counts toward daily limit" — the `exemptFromDaily` flag {#exempt-from-daily}
Per-app-per-profile flag (`app_policy_assignments.exempt_from_daily`, `NOT NULL DEFAULT TRUE` — `V28__apps.sql:36`). **The SPA label is the INVERSE of the model field**: the checkbox reads "Counts toward daily limit" and inverts on write (`web/src/pages/ProfilesPage.tsx:2148,2150,2154`), so *checked = counts = `exemptFromDaily: false`*, and the default (exempt) renders **unchecked**. Two effects, both server-side: (a) exempt traffic is filtered out of the profile's daily total *before* the cap is evaluated — mode-agnostic, Allowed / Blocked / TimeLimited alike (`ProfileAppDispositions.exemptPatterns`, `api/src/policy/ProfileAppDispositions.scala:30-36`); (b) an exempt app still **under its own per-app cap** is carved into `extraAllowed`, which beats a Paused / Schedule / daily-limit whole-MAC block (`PolicyService.exemptUnderCapHosts`, `api/src/policy/PolicyService.scala:1573-1590`). Do **not** shorten (b) to "`extraAllowed` beats `blocked`": for a **non**-exempt app the carve is *suppressed* once the daily cap is exhausted (`ProfileAppDispositions.scala:147`, prose at :109), so the cap still bites. `dailyLimitMinutes = None` means *no cap configured*, hence never exhausted (#1627) — which is why **removing** an app's time limit can make an exempt app reachable during a daily-limit block. Authoritative decision table (rows 9a–9c) and the `allowed_during` (reachability) vs `exemptFromDaily` (budget) orthogonality: → see [`docs/design/per-app-schedules.md`](docs/design/per-app-schedules.md#5-precedence--interaction-table)
...
### Grafana Cloud stack
`https://wifihaven.grafana.net` hosts app metrics (Alloy), Render infra OTLP, deploy annotations. Repo secrets `GRAFANA_CLOUD_URL` + `GRAFANA_CLOUD_ANNOTATION_TOKEN`. The API also ships app logs to Grafana Cloud **Loki** — to **query** them for debugging/incidents (LogQL via the Explore UI, or scriptably via `logcli` / the query_range HTTP API using the read-only access-policy token in the operator's local memory), prefer Loki over `render logs` (a fallback). → see [`docs/ops/grafana-cloud.md`](docs/ops/grafana-cloud.md) (§ *Querying Grafana Cloud* `{#querying-logs}`)

### Every React Query key carries the household {#query-cache-scoping}
The SPA's `QueryClient` is one process-wide singleton that outlives a session, so an unscoped key is a cache entry two households SHARE — that is #2603, where household B's session rendered household A's devices/profiles for the full five-minute `staleTime`. Never write an inline array key: every key comes from `qk` (`web/src/api/queryKeys.ts`), which `withHouseholdScope` prefixes with the JWT's `hh` claim. Covers `queryKey:`, `get`/`setQueryData`, and the `queryKey:` inside an invalidation filter. `useAuth` also `queryClient.clear()`s on login and logout (the primary fix); the scoping is the defence in depth, so it only holds if complete. Enforced by `no-restricted-syntax` in `web/.eslintrc.cjs`. → see [`docs/process/query-cache-scoping.md`](docs/process/query-cache-scoping.md)
