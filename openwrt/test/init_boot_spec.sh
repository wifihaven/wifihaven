#!/bin/sh
# Shell-level smoke tests for openwrt/files/etc/init.d/wifihaven-boot (#308).
#
...
# `inet wifihaven_boot` default-deny table from /usr/share/wifihaven/boot.nft
...
PASS=0; FAIL=0
INIT="$(cd "$(dirname "$0")/.." && pwd)/files/etc/init.d/wifihaven-boot"
...
# packets can leak in the gap before wifihaven-boot loads. This is the
...
grep -q 'nft -f /usr/share/wifihaven/boot.nft' "$INIT" \
  && check "loads /usr/share/wifihaven/boot.nft via nft -f" ok \
  || check "loads /usr/share/wifihaven/boot.nft via nft -f" "expected exact `nft -f /usr/share/wifihaven/boot.nft` invocation"
...
# `service wifihaven-boot stop` after that point is a no-op. We still allow
