...
-- ── global infra allowlist (#1307) ──────────────────────────────────────────
--
-- Prod miss (2026-06-01): when the Kids profile hit its daily limit, the
-- whole-MAC @blocked_macs drop also killed transitive infrastructure hosts the
-- allowed app depends on (iOS/Android connectivity check, OCSP, Apple/Google
-- CDN/PKI), so the app *appeared* blocked though its own domain was reachable.
-- The fix is a curated, network-wide allowlist shipped as a top-level snapshot
-- field `infraAllow`. The router declares one v4 + one v6 set populated by
-- dnsmasq, and carves it out of the whole-MAC block path (drop + DNAT) for ALL
-- macs — same precedence as extraAllowed, but applied universally rather than
-- per-(mac,app). It does NOT carve out of targeted per-host (eb_) / category
-- (bl_) / blockIpOnly drops, which remain intentional.
describe("render global infra allowlist (#1307)", function()
  local function snap_infra(extraAllowed)
    return {
      etag        = "sha256:infra",
      generatedAt = "2026-06-01T17:00:00Z",
  ...
  it("emits the dnsmasq nftset= populator for each infra host", function()
    local conf = render.dnsmasq(snap_infra())
    assert.truthy(conf:find(
      "nftset=/connectivitycheck.gstatic.com/4#inet#wifihaven#infra_allow,6#inet#wifihaven#infra_allow6",
      1, true))
    assert.truthy(conf:find(
      "nftset=/g.aaplimg.com/4#inet#wifihaven#infra_allow,6#inet#wifihaven#infra_allow6",
      1, true))
  end)

  it("declares the v4 + v6 infra_allow sets", function()
    local nft = render.nft(snap_infra())
    assert.truthy(nft:find("set infra_allow {", 1, true))
    assert.truthy(nft:find("set infra_allow6 {", 1, true))
  end)

  it("carves infra_allow out of a TimeLimit @blocked_macs drop (no extraAllowed)", function()
    local nft = render.nft(snap_infra())
    -- A blocked MAC with no extraAllowed is normally a single family-agnostic
    -- drop; with infra enabled it splits per-family to carry the carve-out.
    assert.truthy(nft:find(
      "ether saddr aa:bb:cc:11:22:33 ip daddr != @infra_allow log group 1 counter drop comment \"wh_drop:aa:bb:cc:11:22:33:TimeLimit\"",
      1, true))
    assert.truthy(nft:find(
      "ether saddr aa:bb:cc:11:22:33 ip6 daddr != @infra_allow6 log group 1 counter drop comment \"wh_drop:aa:bb:cc:11:22:33:TimeLimit\"",
      1, true))
  end)

  it("carves infra_allow out of the @blocked_macs DNAT", function()
    local nft = render.nft(snap_infra())
    -- The MAC has no extraAllowed → stays in the @blocked_macs set, which
    -- drives the DNAT. The DNAT line must still let infra hosts reach the net.
    assert.truthy(nft:find(
      "ether saddr @blocked_macs ip daddr != @infra_allow tcp dport 80 dnat ip to 127.0.0.1:8081",
      1, true))
    assert.truthy(nft:find(
      "ether saddr @blocked_macs ip6 daddr != ::1 ip6 daddr != @infra_allow6 tcp dport 80 dnat ip6 to ::1:8081",
      1, true))
  end)

  it("composes infra_allow with a per-(mac,host) extraAllowed exception", function()
    local nft = render.nft(snap_infra({ "mathacademy.com" }))
    assert.truthy(nft:find(
      "ether saddr aa:bb:cc:11:22:33 ip daddr != @ea_aa_bb_cc_11_22_33_mathacademy_com ip daddr != @infra_allow log group 1 counter drop comment \"wh_drop:aa:bb:cc:11:22:33:TimeLimit\"",
      1, true))
    assert.truthy(nft:find(
      "ether saddr aa:bb:cc:11:22:33 ip daddr != @ea_aa_bb_cc_11_22_33_mathacademy_com ip daddr != @infra_allow tcp dport 80 dnat ip to 127.0.0.1:8081",
      1, true))
  end)

  it("preserves the family-agnostic drop when infraAllow is empty", function()
    local s = snap_infra()
    s.infraAllow = {}
    local nft = render.nft(s)
    assert.is_nil(nft:find("set infra_allow {", 1, true))
    -- No carve-out clause; blocked MAC w/o ea stays a single family-agnostic drop.
    assert.truthy(nft:find(
      "ether saddr aa:bb:cc:11:22:33 log group 1 counter drop comment \"wh_drop:aa:bb:cc:11:22:33:TimeLimit\"",
      1, true))
    assert.truthy(nft:find(
