...
-- #353: per-MAC "resolved IP" sets, populated by dnsmasq at DNS resolve
-- time, scoped to that MAC via a dhcp-host tag. A device with
-- BlockRules.blockIpOnly=true gets a forward-chain drop on any daddr NOT
-- in its resolved_<mac> / resolved6_<mac> set — enforcing "use the LAN
-- resolver or you're dropped."
...
local function resolved_tag_name(mac)
...
function M.dnsmasq(snapshot)
  ...
  -- #353: which MACs need a per-MAC resolvetag (blockIpOnly=true). Lookup
  -- by MAC so the dhcp-host loop below can append the tag in one pass.
  local bio_macs = {}
  ...
  -- #353: per-MAC resolved-IP nftset populators. Tag-scoped (no /<domain>/),
  -- so every A/AAAA answered to a client with this tag populates the set.
  -- A → resolved_<mac>; AAAA → resolved6_<mac>. The forward chain then
  -- drops daddrs NOT in the per-MAC set when blockIpOnly is true.
  local bio_sorted = block_ip_only_macs(snapshot)
  if #bio_sorted > 0 then
    emit("# blockIpOnly → tag-scoped per-MAC resolved-IP nftsets (#353)")
    for _, mac in ipairs(bio_sorted) do
      emit(string.format(
        "nftset=tag:%s,4#inet#familydns#%s,6#inet#familydns#%s",
