...
function M.dnsmasq(snapshot, opts)
  ...
  -- #2381: enforcement escape hatch — the dnsmasq half of the M.nft
  -- short-circuit. With enforcement disabled the permissive nft ruleset
  -- declares none of the eb_/bl_/ea_/resolved_/global ipsets, so the conf
  -- MUST carry NO nftset= populators (they would fail against absent sets) and
  -- NO blockEncryptedDns NODATA lines (encrypted-DNS blocking is enforcement
  -- too). dhcp-host= tags are dropped as well — they only exist to scope
  -- per-profile nftset callbacks that no longer exist. dnsmasq is then a plain
  -- recursive resolver and all forwarded traffic passes. A header-only file is
  -- a valid conf-dir fragment. Independent of snapshot freshness (works with a
  -- stale/absent snapshot — the API-down case). See docs/escape-hatch.md.
  if opts.enforcement_disabled then
...
function M.nft(snapshot, opts)
  ...
  -- #2381: enforcement escape hatch. When the local UCI flag
  -- wifihaven.settings.enforcement_disabled is set the agent passes
  -- opts.enforcement_disabled=true; render ONLY the accounting sets/chains
  -- above (so per-device usage still accrues) and NONE of the enforcement
  -- plane below — no blocked_macs / eb_ / bl_ / resolved_ / global sets, no
  -- wifihaven_block forward-drop chain, and no wifihaven_block_nat block-page
  -- DNAT chain — so ALL forwarded traffic passes. Closing the table here and
  -- returning is a SINGLE, structurally complete teardown: every drop set and
  -- every drop/DNAT rule is emitted strictly BELOW this point, so none can
  -- survive the short-circuit (no drop left behind). This is a sanctioned,
  -- narrow local override of "the router is a dumb applier" (AGENTS.md Truth
  -- #2), like the #1911 blockEncryptedDns exception — NOT policy logic. It is
  -- deliberately independent of snapshot freshness (it fires identically for a
  -- stale or absent snapshot), which is the whole point of an OFFLINE hatch:
  -- it works when the API is unreachable. See docs/escape-hatch.md. The
  -- server-driven equivalent (#2382) needs the API up; this one does not.
  if opts and opts.enforcement_disabled then
    emit("}")
    emit("")
    return table.concat(out, "\n")
  end
