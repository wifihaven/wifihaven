...
- **App exempt-from-daily (`exemptFromDaily`)** — per-(app, profile) flag on the policy assignment (`app_policy_assignments.exempt_from_daily`, `NOT NULL DEFAULT TRUE` — `V28__apps.sql:36`; the semantic originates in #15 / `V6__site_time_exempt_from_daily.sql`, which added the column to the predecessor `site_time_limits` table, migrated forward by `V35__migrate_legacy_into_apps.sql:36-42`) answering "does this app's time spend the profile's daily budget?". Two effects, both computed server-side:
  1. **Excluded from the profile daily total.** `ProfileAppDispositions.exemptPatterns` (`api/src/policy/ProfileAppDispositions.scala:30-36`) collects the hosts of every exempt assignment and `TimeStatusService` filters that traffic out *before* summing the daily total (`TimeStatusService.scala:641-642,792,913`). It is **independent of `mode`** — Allowed, Blocked and TimeLimited assignments are all eligible — and independent of any per-instant schedule state.
  2. **Carve-out around a whole-MAC block.** `PolicyService.exemptUnderCapHosts` (`api/src/policy/PolicyService.scala:1573-1590`) puts an exempt app that is still under *its own* per-app cap into `extraAllowed`, so it survives a Paused / Schedule / daily-limit block (#1105, #1515). `dailyLimitMinutes = None` means *no per-app cap configured*, therefore never exhausted (#1627) — an exempt app with no cap always carves, which is why **removing** an app's time limit can make it reachable during a daily-limit block.

  The carve is **not** unconditional: for a **non**-exempt app with an active `allowed_during` window it is suppressed once the profile's daily cap is exhausted (`if (!(capExhausted && !d.exemptFromDaily) && ...)`, `ProfileAppDispositions.scala:147`; prose at `:109`), so the daily limit still bites. Do not compress this to a bare "`extraAllowed` beats `blocked`" — that is the *router* enforcement invariant (#421), not a statement that the server always carves.

  > **The SPA label is the inverse of the field.** The checkbox reads **"Counts toward daily limit"** and inverts on write — `checked={!(current?.exemptFromDaily ?? true)}` / `onChange={e => toggleExempt(!e.target.checked)}` (`web/src/pages/ProfilesPage.tsx:2148,2150,2154`). So *checked = counts against the cap = `exemptFromDaily: false`*, and the `DEFAULT TRUE` (exempt) state renders **unchecked**. The `ScheduleRuleEditor` variant states the same flag in the non-inverted direction ("Exempt from the daily time limit…", `ProfilesPage.tsx:2306-2313`).

  Authoritative decision table (rows 9a–9c) and the orthogonality between `allowed_during` (the *reachability* axis) and `exemptFromDaily` (the *budget* axis): [`docs/design/per-app-schedules.md` §5](design/per-app-schedules.md#5-precedence--interaction-table).
...
3. Daily time limit reached (`time_used_today >= daily_minutes + extensions_today`) → `blocked = true`, reason `TimeLimit`
   - Traffic belonging to an app whose assignment is `exemptFromDaily` is **filtered out of `time_used_today` before this comparison** (`ProfileAppDispositions.exemptPatterns` → `TimeStatusService.usedSecondsForProfile`, `api/src/policy/ProfileAppDispositions.scala:30-36`, `TimeStatusService.scala:641,792`), so exempt app time never consumes the daily cap. The exclusion is mode-agnostic. Separately, `PolicyService.exemptUnderCapHosts` contributes an exempt app that is still under its own per-app cap to the same `extraAllowed` field that step 6 below feeds (a different producer, not step 6 itself), so it stays reachable *through* a `TimeLimit` (or `Paused` / `Schedule`) block — see **App exempt-from-daily** under "Key domain concepts", and note the carve is suppressed for a **non**-exempt app once the cap is exhausted.
