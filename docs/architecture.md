...
- **App exempt-from-daily (`exemptFromDaily`)** — per-(app, profile) flag on the policy assignment (`app_policy_assignments.exempt_from_daily`, `NOT NULL DEFAULT TRUE` — `V6__site_time_exempt_from_daily.sql:11`, `V28__apps.sql:36`) answering "does this app's time spend the profile's daily budget?". Two effects, both computed server-side:
