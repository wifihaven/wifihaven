...
| ~~`HouseholdSettingsRepo.get` (single row)~~ | — | **CLOSED (#2533, #2553).** `get` and the unscoped `update(s)` are deleted; `getForHousehold(household)` / `update(household, s)` are the only accessors. #2553 removed the last two `HouseholdId.Default` reads: `TimeUsedRollupJob` / `AmbientLearnJob` now read each household's OWN settings inside their per-household loop (own day key, own heartbeat/ambient knobs), and `update`'s rollup-cache invalidation is scoped to the writing household's profiles. |
