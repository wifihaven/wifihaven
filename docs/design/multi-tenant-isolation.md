...
| ~~`ProfileRepo.listAll`~~ | — | **CLOSED (#2257).** Deleted with no replacement; `listAllForHousehold(household)` is the only accessor. |
...
| ~~`DeviceRepo.listAll`~~ | — | **CLOSED (#2257).** Deleted with no replacement; every `DeviceRepo` method now takes a `household`. |
