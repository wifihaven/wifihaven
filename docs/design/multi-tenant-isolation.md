...
| ~~`ProfileRepo.listAll`~~ | — | **CLOSED (#2257).** Deleted; `listAllForHousehold(household)` is the only accessor — no cross-tenant successor. |
...
| ~~`NamedScheduleRepo.listAll`~~ | — | **CLOSED (#2126).** Scoped to `listAllForHousehold(household)`. |
...
| ~~`DeviceRepo.listAll`~~ | — | **CLOSED (#2257).** Deleted; no cross-tenant successor. No `DeviceRepo` method returns rows across the tenant boundary — the two that take no `household` are scoped by their argument instead: `listForProfile(profileId)` through the profile FK, and `findOwningHousehold(mac)` exists precisely to answer "which household?" at ingest. |
