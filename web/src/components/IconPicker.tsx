...
export function IconPicker({ value, onChange, hosts = [] }: Props) {
  ...
  return (
    <div className="space-y-3">
      <div role="tablist" className="inline-flex rounded-xl bg-brand-surface border border-brand-border p-1">
        {(['emoji', 'url', 'favicon'] as const).map(m => (
          <button
            ...
            className={`px-3 py-1 text-xs font-medium rounded-lg transition-colors ${
              mode === m ? 'bg-brand-accent text-white' : 'text-brand-text hover:text-brand-ink'
            }`}
          >
      ...
      <div className="flex items-center gap-3">
        <div className="w-12 h-12 flex items-center justify-center bg-brand-surface border border-brand-border rounded-xl shrink-0">
          <AppIcon
        ...
        <div className="flex-1 min-w-0">
          {mode === 'emoji' && (
            <input
              ...
              className="w-full bg-brand-surface border border-brand-border-strong rounded-xl px-4 py-2 text-brand-ink focus:outline-none focus:border-brand-accent"
            />
          )}
          {mode === 'url' && (
            <input
              ...
              className="w-full bg-brand-surface border border-brand-border-strong rounded-xl px-4 py-2 text-brand-ink text-sm focus:outline-none focus:border-brand-accent"
            />
          )}
          {mode === 'favicon' && (
              ...
              <button
                ...
                className={`inline-flex items-center gap-2 px-2.5 py-1.5 rounded-lg border text-xs font-mono transition-colors ${
                  selected
                    ...
                    : 'bg-brand-surface border-brand-border-strong text-brand-text hover:border-brand-border-strong'
                }`}
              >
                <img
                  src={url}
