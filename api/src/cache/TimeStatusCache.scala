        ...
        case None    =>
          ...
          Promise.make[Throwable, AnyRef].flatMap { mine =>
            Option(inFlight.putIfAbsent(key, mine)) match {
              ...
              case None           =>
                // We own the slot. Compute the owned result, then ALWAYS free the slot and publish
                // the outcome to followers via a single `onExit` — even on failure or interrupt —
                // so the in-flight entry can never wedge waiters or leak regardless of which path
                // (re-check hit vs. load) ran.
                //
                ...
                // winner does `cache.put` BEFORE it frees its slot (both inside this same `onExit`),
                // so if the slot was free for us to win, any value it produced is already in the
                // cache. Re-check before loading: on a hit we skip the redundant build entirely
                // (the shared `onExit` then publishes the cached value to any follower that grabbed
                // `mine`). This closes the window where a late follower would otherwise launch a
                // second `load` (`count == 2`). A prior winner that *failed* leaves the cache empty,
                // so we correctly fall through and re-load.
                ZIO
                  .succeed(Option(cache.getIfPresent(key)))
                  .flatMap {
                    case Some(v) => ZIO.succeed(hits.increment()).as(v.asInstanceOf[V])
                    case None    =>
