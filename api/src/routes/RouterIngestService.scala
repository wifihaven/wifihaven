...
final class RouterIngestService(
  ...
  def ingestUsage(router: Router, body: String): IO[ApiError, Unit] =
    LogContext.annotate(LogContext.RouterId, router.id.toString) {
      for {
        ...
        // #2108: read this router's household settings (daily-reset tz etc.) and thread the
        // household into every MAC-keyed write so usage accretes only onto its own tenant's rows.
        settings <- householdSettingsRepo
          .getForHousehold(router.householdId)
          .mapError(ApiError.Db(_))
        _        <- handleUsage(router.id, router.householdId, ps, pe, records, settings)
  ...
  def ingestEvents(router: Router, body: String): IO[ApiError, Unit] =
    LogContext.annotate(LogContext.RouterId, router.id.toString) {
      for {
        ...
        _ <- handleEvents(router.id, router.householdId, events)
        _ <- routerRepo.touch(router.id, None, None).mapError(ApiError.Db(_))
      } yield ()
    }
  ...
  private def handleUsage(
    ...
    for {
      ...
      _        <- ZIO.when(newCount > 0)(
        applyDelta(routerId, household, periodEnd, rewritten, settings),
      )
    } yield ()
  }
  ...
  private def applyDelta(
    ...
    // #2108: both MAC-keyed writes carry the router's household so a router only ever writes its own
    // tenant's time_usage / devices rows (design §3.2.2).
    timeUsageRepo
      .incrementSecondsAndBytesBatch(increments, household)
      .mapError(ApiError.Db(_)) *>
      deviceRepo
        .touchLastSeenBatch(records.map(r => (r.mac, r.ip)).distinct, periodEnd, household)
        .mapError(ApiError.Db(_))
        .unit
  }
  ...
  private def handleEvents(
    ...
    for {
      ...
      _         <- ZIO.foreachDiscard(events)(applyDhcpOrFirstSeen(household, _))
      ...
      e.mac match {
        ...
        case Some(mac) =>
          for {
            ...
            // #2108: resolve, touch, rename, and create the device all within the router's household
            // (design §3.2.2). A same-MAC row in another household is invisible here, so first-seen
            // discovery creates THIS household's own row — never lookup-and-reject.
            existing <- deviceRepo.findByMacInHousehold(mac, household).mapError(ApiError.Db(_))
            _        <- existing match {
              case Some(_) =>
                deviceRepo
                  .touchLastSeen(mac, e.ip, ts, household)
                  .mapError(ApiError.Db(_))
                  .unit *>
                  ZIO
                    .foreachDiscard(e.hostname)(h =>
                      deviceRepo
                        .renameIfAutoGenerated(mac, h.value, household)
                        .mapError(ApiError.Db(_)),
                    )
              case None    =>
