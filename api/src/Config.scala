...
//                             the agent's ONLY credential; thread- + household-bound, expiring.
//                             SECRET, generated by the operator (≥32 chars recommended).
//   - `agentTokenTtlMinutes`  agent-token lifetime. Must OUTLIVE a paused cloud-agent run (#2473 —
//                             see [[AgentTokenTtl]] for the sizing and the security trade-off).
//                             Clamped to ≥1.
...
/**
 * #2473 — the default lifetime of a per-session cloud-agent token ([[wifihaven.api.support
 * .ConsentToken]] and [[wifihaven.api.press.PressToken]]). ONE literal for both channels: the
 * constraint that sizes it is a property of the shared cloud-agent transport, not of either
 * audience, so a single value cannot drift into "support was fixed, press still expires".
 *
 * WHY 24 HOURS. The token is minted when a message is dispatched and redeemed when the cloud agent
 * posts its answer back — so it must outlive the WHOLE round trip, including the transport's own
 * pauses. A `claude-code-cloud` routine run can be **suspended on subscription usage limits and
 * resumed later**: the rolling limit window is hours long, and a pause that starts in the evening
 * resumes the next morning. The previous 30-minute value was sized for a synchronous dispatch and
 * was shorter than the transport's own retry latency, so on 2026-07-27 a resumed support run posted
 * its reply 2.5h after mint, got 401, and the customer's answer was silently lost (#2473). Anything
 * under ~6h reproduces that. 24h is the round ceiling that clears the rolling window with margin
 * and survives an overnight pause, while still being a bound rather than "forever".
 *
 * SECURITY TRADE-OFF — stated, not hand-waved. A longer TTL widens the window in which a LEAKED
 * token is usable, so **TTL is no longer the primary bound on a leaked token**. The #2241 model
 * does not rest on it: every other compensating control is unchanged and is what actually carries
 * the model —
 *   - **single household** — the support token CARRIES its household and the read derives scope
 *     FROM the verified token, so cross-tenant reads are impossible by construction;
 *   - **thread-/reply-target-bound** — the reply goes into THAT Plain thread (support) or to THAT
 *     email address (press), never one named by the request or the agent;
 *   - **read-only + narrowly actioned** — it authorises only the enumerated agent endpoints; there
 *     is no mutation path behind it;
 *   - **consent-scoped** — the household read is refused outright unless the customer had
 *     explicitly granted data access (#2419); most tokens carry no data scope at all;
 *   - **out-of-band** — minted into the agent kickoff, never into customer-visible text;
 ...
 * the containerised deploy renders its own `application.conf` from `docker/entrypoint.sh` and does
 * NOT template these keys, so on Render this default IS the effective value — there is no env-var
 * stopgap, deliberately: a second copy of the number in the entrypoint template would be exactly
 * the drift this single constant exists to prevent.
 */
object AgentTokenTtl {
  val DefaultMinutes: Int = 24 * 60
}
...
case class SupportConfig(
    ...
    // #2473: sized to OUTLIVE a usage-limit-paused cloud-agent run — see [[AgentTokenTtl]] for the
    // sizing rationale and the security trade-off (TTL is no longer the primary bound).
    agentTokenTtlMinutes: Int = AgentTokenTtl.DefaultMinutes,
...
//                           expiring). SECRET.
//   - `agentTokenTtlMinutes` press-token lifetime; the SAME [[AgentTokenTtl]] default as support
//                           (#2473 — a paused cloud-agent run is a transport property, not an
//                           audience one). Clamped to ≥1.
...
case class PressConfig(
    ...
    claudeCodeRoutineToken: String = "",
    agentTokenSecret: String = "",
    // #2473: the SAME [[AgentTokenTtl]] default as support — the sizing constraint is a property of
    // the shared cloud-agent transport (a run paused on usage limits and resumed hours later), not
    // of either audience, so the two channels cannot drift apart.
    agentTokenTtlMinutes: Int = AgentTokenTtl.DefaultMinutes,
    agentApiBase: String = "https://api.wifihaven.net",
    deploymentEnv: String = "",
    ...
    // subdomain is not a verified Resend SENDING domain. Required when the responder is enabled —
    // no dark-by-default (#2265).
