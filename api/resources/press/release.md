...
# remain unresolved): date, founderName, betaSignupUrl, pressKitUrl.
#
# The dateline carries NO city (operator decision 2026-08-15 — the product is internet-wide,
# not local news). The DATE stays a token: it is the one field whose correctness is
# time-dependent, so a send that slips a day must be stopped by the unresolved-token guard
# rather than carry a stale dateline. Supply "August 17, 2026" for the launch send.
