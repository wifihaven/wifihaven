...
/**
 * #958 (URL-sourced lists): pure parser tests for the upstream payload formats. No network access —
 * `BlocklistFetcher.Parser` is exposed so we can feed it canned strings.
 */
object BlocklistFetcherSpec extends ZIOSpecDefault {
  ...
  def spec = suite("BlocklistFetcher.Parser")(
    test("hosts-file: 0.0.0.0 host and 127.0.0.1 host lines") {
      ...
      assertTrue(
        parsed == List(Hostname.unsafe("doubleclick.net"), Hostname.unsafe("tracker.example.com")),
      )
    },
    ...
    test("hosts-file: whole-line and trailing comments are stripped") {
      ...
      val parsed = BlocklistFetcher.Parser.parse(body, BlocklistFormat.HostsFile).toSet
      assertTrue(
        parsed == Set(Hostname.unsafe("doubleclick.net"), Hostname.unsafe("tracker.example")),
      )
    },
    ...
    test("domain-list: drops obviously-bad lines without failing") {
      ...
      val parsed = BlocklistFetcher.Parser.parse(body, BlocklistFormat.DomainList).toSet
      assertTrue(
        parsed == Set(Hostname.unsafe("valid.example"), Hostname.unsafe("another-valid.example")),
      )
    },
  )
}
